...
# fit scale(X_train) and scale(X_test) to PCA model
# project principle components as scaled_X_train and scaled_X_test
pca = PCA(n_components=4)
scaled_X_train = pca.fit_transform(scale(X_train))
scaled_X_test = pca.fit_transform(scale(X_test))

# fit scaled_X_train to LR model
LR_baseRent = LinearRegression().fit(scaled_X_train, baseRent_train)

# predict values for scaled_X_test
baseRent_pred = LR_baseRent.predict(scaled_X_test)
